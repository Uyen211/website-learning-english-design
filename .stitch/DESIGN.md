...
To maintain the playful, modern, and friendly atmosphere of DiveVerse, we use **Lucide Icons** (specifically the 67 SVGs stored in `design-system/icon-library.json`). 

### Principles of Icon Usage
1. **Purposeful, Not Decorative:** Do not overuse icons. An icon should either clarify an action (e.g., `play`, `pause`, `x` to close), indicate a status (e.g., `check-circle`, `flame` for streak), or anchor main navigation (e.g., `layout-dashboard`).
2. **Text Over Icon:** If a button has a clear text label (e.g., "Nộp bài", "Lưu thay đổi"), it generally does **not** need an icon unless it's a primary CTA where a directional arrow (`arrow-right`) adds momentum.
3. **Consistent Style:** 
   - **Size:** Base size is `20px` for inline UI elements and buttons. `16px` for small badges. `32px` to `48px` for hero or empty states.
   - **Stroke Weight:** Strictly `2px`.
   - **Corner Style:** Rounded caps and joins (`stroke-linecap="round" stroke-linejoin="round"`).
   - **Color:** Always use `currentColor` to inherit the surrounding text color (e.g., `{colors.ink}` or `{colors.brand-peach}`). Never hardcode colors in the SVG.

### Specific Assignments
- **Navigation:** `house` (Dashboard), `dumbbell` (Practice), `graduation-cap` (Exams), `search` / `book-open` (Dictionary).
- **Gamification:** `flame` (Streak), `trophy` / `star` (Achievements), `circle-check` (Completion).
- **Learning Tools:** `audio` / `headphones` (Listening), `mic` (Speaking), `sparkles` / `lightbulb` (AI/ZPD Feedback).
- **System Actions:** `x` (Close/Cancel), `chevron-left/right` (Back/Next), `ellipsis` (More Options).
