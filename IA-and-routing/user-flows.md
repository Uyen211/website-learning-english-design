...
`Unit Detail/Dashboard` → click Test → `Pre-test Page` (Hiển thị cấu trúc, thời gian) → Chọn mode (`Tập dượt` hoặc `Thi thật`) → `Start`:
*   **Mini/Level Test:** Làm tuần tự các Tab kỹ năng.
*   **Mock Test (Strict Mode):** `Listening` (Auto-next, không pause) → `Reading` → `Writing` → `Speaking` (AI Examiner).
→ `Auto-Submit` khi hết giờ hoặc nhấn Nộp bài → `Test Result Page` (Band score/Điểm chi tiết/Giải thích) → `Exit`.
